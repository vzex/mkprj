...
(defun project-find-tag (input)
  ...
  (let* (
	 ...
	 (regexp_re (concat regexp ".*(\^?[0-9]"))
	 )
    (save-excursion
      (let ((first-time t))
        (while (visit-tags-table-buffer (not first-time))
               (setq first-time nil)
               (setq count (etags-tags-apropos_ regexp_re))
               )))
...
(defun project-find-tag-exact (input)
  ...
  (let* (
	 ...
	 (regexp_old (concat "\\<" regexp_ "\\>"))
		 (regexp (concat "\\<" regexp_ "\\>(\^?[0-9]"))
		 )
    (save-excursion
      (let ((first-time t))
        (while (visit-tags-table-buffer (not first-time))
    ...
    ;(message "final count %d" count)
(if (= count 0)
  (error "No tag contains the keyword:%s" regexp_old)
  (if (= count 1)
    (let* ((buf (find-tag-noselect regexp_old nil t))
           (pos (with-current-buffer buf (point))))
      (condition-case nil
                      (switch-to-buffer buf)
                      (error (pop-to-buffer buf)))
